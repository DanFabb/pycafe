...
# What the meshes in that folder are cut to, said where the file is picked.
# A mesh carries its own element size, so this is the one thing about the
# file that the dialog cannot show and the user has to be told: how high in
# frequency the file being chosen is allowed to go.
LIBRARY_TARGET_F_MAX = 1500.0
LIBRARY_ELEMENTS_PER_WAVELENGTH = 6

# The dialog prompt. Both panels put this at the top of the open window -
# AppleScript as the prompt, Tk as the window title - so it has to say the
# whole thing in one line.
LIBRARY_RESOLUTION_PROMPT = (
    "pyCAFE — select a gmsh mesh (.msh): every mesh in Library/ is cut for "
    "6 elements per wavelength up to 1500 Hz, except cavity_plate_FG.msh "
    "(Fahy & Gardonio's own 16x16x8 mesh, 500 Hz)"
)

# The same thing with room to explain itself, printed to the console just
# before the panel opens, since a file dialog has nowhere to put a
# paragraph.
LIBRARY_RESOLUTION_NOTE = """\
The meshes in Library/ are cut to one rule: six elements per acoustic
wavelength up to 1500 Hz, i.e. no fluid element longer than
h = c0 / (6 * 1500) = 38.1 mm. So any of these files can be asked for an
f_max up to 1500 Hz and pyCAFE will not refuse it.

  one exception   cavity_plate_FG.msh keeps Fahy & Gardonio's own
                  16 x 16 x 8 mesh (45 mm elements), because reproducing
                  their eighteen coupled modes means reproducing their
                  mesh. It stops at 1270 Hz, and the forced version of
                  that case is cavity_plate_FG_directresponse.msh, which
                  is cut to the rule.
  six is a floor  not an accuracy target: the trilinear element disperses
                  as 164 / n^2, about 4.6% at six per wavelength. Thirteen
                  per wavelength is the 1% figure, and on a 38.1 mm mesh
                  that is only ~690 Hz. Ask for a lower f_max, or a finer
                  mesh, when the whole band has to be worth 1%.
  the plate       is not covered. Its bending wavelength is far shorter
                  than the acoustic one - 81 mm for 1 mm aluminium at
                  1500 Hz - so a coupled run near the top of the band is
                  resolving the fluid, not the panel."""
...
def ask_for_geometry(*, title=None, initial_dir=None, fluid=None,
    ...
    **This release reads gmsh meshes only.** A ``.msh`` carries its own
    ...
    guessed. Which is why the dialog says what those element sizes are:
    every mesh in ``Library/`` is cut for **six elements per acoustic
    wavelength up to 1500 Hz** (38.1 mm), so any of them can be given an
    ``f_max`` that high without being refused. The one exception is
    ``cavity_plate_FG.msh``, which keeps Fahy & Gardonio's own
    16 x 16 x 8 mesh and stops at 1270 Hz. The same text goes to the
    console before the panel opens, where there is room for the caveats
    — see :data:`LIBRARY_RESOLUTION_NOTE`. Meshing a CAD file and reading a Nastran deck are written
    ...
    **kwargs
    ...
    if verbose:
        print(LIBRARY_RESOLUTION_NOTE)
    path = ask_for_file(
        "geometry",
        title=title or LIBRARY_RESOLUTION_PROMPT,
...
def ask_for_structure(*, materials=("aluminium", "steel"),
    ...
    materials : sequence of str, optional
        ...
        :mod:`pycafe_vibro.structure` (``aluminium``, ``steel``).
    supports : sequence of str, optional
        Which supports are offered; see
        :func:`~pycafe_vibro.prepare_vibroacoustic_system.prepare_vibroacoustic_system`
    ...
    Structure
        ...
        Asking this needs ``pycafe_vibro`` installed: the materials it
        offers are structural.
    ...
    """
    try:
        from pycafe_vibro import structure as structure_module
    except ImportError:
        raise ImportError(
            "asking what the structure is made of needs pycafe_vibro: "
    ...
    factory = getattr(structure_module, material, None)
    if factory is None:
        raise RuntimeError(
            f"{material!r} is not a material of pycafe_vibro.structure; "
