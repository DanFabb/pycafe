...
acoustic-structure coupling needs: the plate quadrilaterals come out as
...
def duct_with_flush_plate(
    ...
    *,
    ...
    with GmshModel("duct_flush_plate", verbose=verbose) as m:
        ...
        # ceil, not round: h is the largest element allowed, so a curve
        # that is 1.01 h long is cut in two. Rounding to nearest let a
        # curve come back with elements longer than h, and a mesh asked
        # for six elements per wavelength then delivered 5.9.
        for _, curve in m.model.getEntities(1):
            bb = np.array(m.model.getBoundingBox(1, curve))
            n = max(2, int(math.ceil(float(np.linalg.norm(bb[3:] - bb[:3])) / h)) + 1)
