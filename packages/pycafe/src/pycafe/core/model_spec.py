...
def _structure_class():
    """
    The :class:`Structure` of ``pycafe_vibro``, or a clear error.

    The fluid of a model is acoustic and defined above; the shell that
    bounds it is structural, and ships with ``pycafe_vibro``. Nothing
    in an acoustic-only model reaches this.
    """
    try:
        from pycafe_vibro.structure import Structure
    except ImportError:
        raise ImportError(
            "this model has a structure, and the structural half of "
            "pyCAFE is a separate package: pip install pycafe-vibro."
        ) from None
    return Structure
...
class CadFile:
    ...
    **which entity is the fluid, which the structure**. Run
    ...
    independent_structure : bool, optional
        ...
        (:mod:`pycafe_vibro.coupling_nonconforming`). This is
...
class ModelSpec:
    ...
    build_coupling : bool, optional
        Assemble the acoustic-structure coupling matrix. Default True.
    verbose : bool, optional
    ...
    f_max: float
    fluid: Fluid = AIR
    structure: Optional[object] = None   # pycafe_vibro.Structure
    ...
    * a field the spec **leaves out** — no :class:`Structure`, or the
    ...
    if structure is None:
        for pid in source.structure:
            ...
            structure = _structure_class()(**kwargs)
            notes.append(
