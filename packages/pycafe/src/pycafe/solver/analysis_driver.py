...
def run_acoustic_analysis(
    *,
    ...
    elements : dict, optional
        ...
        When given on a 3D mesh, the direct analysis hands the result to
        the 3D post-processing (volume plots, cutting planes and movies)
        instead of the 2D one.
    velocity_operator : NormalVelocityOperator, optional
        Pre-assembled normal-velocity load operator, already reduced to
        the free DOFs (``system["velocity_red_op"]``).
    impedance_operator : ImpedanceOperator, optional
        Pre-assembled impedance operator, already reduced to the free
        DOFs (``system["C_red_op"]``). Needed when the impedance is
        frequency dependent.
