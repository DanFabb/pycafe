...
- coupling blocks ``Kc`` and ``Mc = -rho0 Kc^T``, see
  :mod:`pycafe_vibro.coupling` for the sign convention.

The coupled system is then solved by
:mod:`pycafe_vibro.solver_vibroacoustic`.
"""
...
from .domains import build_KM_structural_domain
...
def prepare_vibroacoustic_system(
    *,
    ...
    interface_sign : {+1, -1}, optional
        ...
        :func:`pycafe_vibro.coupling.interface_normals`.
    nonconforming : {"auto", False} or dict, optional
        ...
        :mod:`pycafe_vibro.coupling_nonconforming` and says so
    ...
    system : dict
        ...
        two domains (done by
        :func:`pycafe_vibro.solver_vibroacoustic.build_coupled_blocks`).
        ``system["props"]``      — material/fluid properties used.

    See Also
    --------
    pycafe_vibro.coupling : Coupling matrix and sign convention.
    pycafe_vibro.solver_vibroacoustic : Coupled solvers.
    """
