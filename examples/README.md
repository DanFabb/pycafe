...
| `01_cavity_acoustic_3d.ipynb` | Rigid-walled air cavity picked from the dialog, meshed in `CHEXA8`: the first twelve modes against the analytical modes of a rectangular box. Sides and element counts are read off the mesh, and a geometry that is not such a box — a flexible wall, tetrahedra, a mesh too coarse for the twelfth mode — stops the run instead of being compared with a formula that does not describe it. Geometry: `Library/box_cavity.msh`. |
