...
| `05_shell_in_cavity_nonconforming.ipynb` | A shell inside a curved cavity (`Geom/Vibro_ac1.step`): a geometry no conforming mesh can carry. Fluid and shell are meshed separately (`independent_structure=True`), merged without sharing a node, and coupled by the interpolation method of Mi & Zheng (2018) — with the accuracy measured against a conforming model of the box-plate case. Mesh: `shell_cavity.msh`. |
