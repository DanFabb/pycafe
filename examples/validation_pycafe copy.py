...
====================
Analytical validation of pyCAFE — eigenfrequencies of a 2D rigid rectangular
acoustic cavity (all walls hard / Neumann).
...
- validation_pycafe.tex      : LaTeX document with tables and figures (actual numbers)

Balanced mesh configuration used for the histogram and frequency table:
...
tex_labels = [f"$({x[1]},{x[2]})$" for x in analytic]  # LaTeX math
...
# ── balanced run — frequency table + histogram data ───────────────────────────
...
for case in field_cases:
    ...
    field_case_descriptions.append((case["key"], case["title"], case["description"]))

field_case_tex_blocks = []
for case_key, case_title, case_desc in field_case_descriptions:
    field_case_tex_blocks.append(
        r"""\begin{figure}[p!]
  \centering
  \caption{"""
        + case_title
        + r""". """
        + case_desc
        + r""" Probe location: $(L_x/2, L_y/2)$. Left: $|p|$. Centre: $\Re(p)$. Right: phase of $p$. In each image, pyCAFE is shown as a solid line and FEniCSx as a dashed line.}
  \begin{minipage}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{comparison_direct_"""
        + case_key
        + r"""_abs.png}
  \end{minipage}\hfill
  \begin{minipage}{0.32\textwidth}
...
# store FEniCSx results for LaTeX
_fenics_modal_err_mean = err_fenics.mean()
_fenics_rows = []
for i in range(N_MODES):
    _, m, k = analytic[i]
    _fenics_rows.append(
        f"  $({m},{k})$ & {fmt(f_ref[i])} "
        f"& {fmt(freqs_fenics[i])} & {fmt(err_fenics[i])} "
        f"& {fmt(freqs_cafe[i])} & {fmt(err_cafe[i])} \\\\"
    )

_fenics_rows_zp = []
for i in range(N_MODES):
    _, m, k = analytic_zp[i]
    _fenics_rows_zp.append(
        f"  $({m},{k})$ & {fmt(f_ref_zp[i])} "
        f"& {fmt(freqs_fenics_zp[i])} & {fmt(err_fenics_zp[i])} "
        f"& {fmt(freqs_cafe_zp[i])} & {fmt(err_cafe_zp[i])} \\\\"
    )
...
for i in range(N_MODES):
    _, m, k = analytic[i]
    rows_t1.append(
        f"  $({m},{k})$ & {fmt(f_ref[i])} & {fmt(freqs_q4[i])} & {fmt(err_q4[i])} "
        f"& {fmt(freqs_q8[i])} & {fmt(err_q8[i])} \\\\"
    )

rows_t2 = []
for (h, n4_, e4m, t4_), (_, n8_, e8m, t8_) in zip(conv_q4, conv_q8):
    rows_t2.append(
        f"  {fmt(h, 3)} & {n4_} & {fmt(e4m)} & {t4_:.2f} "
        f"& {n8_} & {fmt(e8m)} & {t8_:.2f} \\\\"
    )
...
\title{\textbf{Analytical Validation of pyCAFE}\\
\large Acoustic Eigenfrequencies of a 2D Rigid Rectangular Cavity\\
...
The test domain is a two-dimensional rectangle
$\Omega=[0,L_x]\times[0,L_y]$ with $L_x="""
+ fmt(Lx, 1) + r"""\,\text{m}$ and $L_y="""
+ fmt(Ly, 1) + r"""\,\text{m}$,
filled with air at 20\,$^\circ$C
($c_0="""   + fmt(c0, 1) + r"""\,\text{m/s}$,
 $\rho="""  + fmt(rho, 3) + r"""\,\text{kg/m}^3$).

All four walls enforce the \textbf{rigid (hard-wall)} condition
\begin{equation}
  \frac{\partial p}{\partial n} = 0 \quad \text{on } \partial\Omega,
\end{equation}
which is the natural boundary condition of the weak formulation and requires
no matrix modification.

The discrete generalised eigenvalue problem is
\begin{equation}
  \mathbf{K}\,\boldsymbol{\varphi}
  = \lambda\,\mathbf{M}\,\boldsymbol{\varphi}, \qquad \lambda = \omega^2,
\end{equation}
where $\mathbf{K}$ and $\mathbf{M}$ are the acoustic stiffness and mass
matrices assembled element-by-element from the quadrilateral shape functions.

The exact eigenfrequencies are
\begin{equation}
  f_{mn} = \frac{c_0}{2}
  \sqrt{\!\left(\frac{m}{L_x}\right)^{\!2}+\left(\frac{n}{L_y}\right)^{\!2}},
  \qquad m,n=0,1,2,\ldots,\quad (m,n)\neq(0,0).
\end{equation}

%%─────────────────────────────────────────────────────────────────────────────
\section{Balanced mesh configuration}

A \emph{balanced} mesh size of $h="""
+ fmt(H_BAL, 3) + r"""\,\text{m}$ is used for the
eigenfrequency comparison and histogram.  This value sits in the middle of
the convergence study range (Section~\ref{sec:conv}) and provides a good
trade-off between accuracy and computational cost:
CQUAD8 already achieves a mean relative error below """
+ fmt(err_q8.mean(), 3) + r"""\,\% at this resolution, while CQUAD4 requires
a finer mesh to reach similar accuracy.

%%─────────────────────────────────────────────────────────────────────────────
\section{Eigenfrequency comparison}

Table~\ref{tab:freqs} reports the first """
+ str(N_MODES) + r""" eigenfrequencies computed by pyCAFE
(CQUAD4: """ + str(n_nodes_q4) + r""" nodes;
 CQUAD8: """ + str(n_nodes_q8) + r""" nodes; balanced mesh $h="""
+ fmt(H_BAL, 3) + r"""\,\text{m}$)
against the analytical solution.
The relative error is
$\varepsilon=|f_\text{FEM}-f_\text{an}|/f_\text{an}\times100\,\%$.

\begin{table}[h!]
\centering
\caption{Eigenfrequency comparison — pyCAFE vs analytical
         (balanced mesh, $h=""" + fmt(H_BAL, 3) + r"""\,\text{m}$).}
\label{tab:freqs}
\begin{tabular}{crrrrr}
\toprule
Mode $(m,n)$ & $f_\text{an}$ [Hz]
             & $f_\text{CQUAD4}$ [Hz] & $\varepsilon_4$ [\%]
             & $f_\text{CQUAD8}$ [Hz] & $\varepsilon_8$ [\%] \\
\midrule
"""
+ "\n".join(rows_t1)
+ r"""
\midrule
\multicolumn{2}{r}{Mean error}
  & \multicolumn{2}{c}{""" + fmt(err_q4.mean()) + r"""\,\%}
  & \multicolumn{2}{c}{""" + fmt(err_q8.mean()) + r"""\,\%} \\
\bottomrule
\end{tabular}
\end{table}

Figure~\ref{fig:hist} shows the same data as a grouped histogram.
The upper panel compares absolute eigenfrequencies; the lower panel shows
the relative error per mode.

\begin{figure}[h!]
  \centering
  \includegraphics[width=\textwidth]{validation_histogram.png}
  \caption{Grouped bar chart of the first 10 eigenfrequencies
           (analytical, CQUAD4, CQUAD8) at the balanced mesh
           $h=""" + fmt(H_BAL, 3) + r"""\,\text{m}$.
           Lower panel: relative error vs analytical solution.}
  \label{fig:hist}
\end{figure}

%%─────────────────────────────────────────────────────────────────────────────
\section{$h$-refinement convergence study}
\label{sec:conv}

For a polynomial order~$p$ element the eigenfrequency error scales as
$\varepsilon\propto h^{2p}$~\cite{Ihlenburg1998}:
\begin{itemize}
  \item CQUAD4 ($p=1$): expected rate $\mathcal{O}(h^2)$,
  \item CQUAD8 ($p=2$): expected rate $\mathcal{O}(h^4)$.
\end{itemize}

Table~\ref{tab:conv} reports the mean relative error over the first
""" + str(N_MODES) + r""" modes for a sequence of mesh sizes.
The balanced configuration ($h=""" + fmt(H_BAL,3) + r"""\,\text{m}$) is used for
the frequency comparisons reported in the previous section.
Figure~\ref{fig:conv} shows the log--log convergence curves with
theoretical reference slopes and per-point solver timing.

\begin{table}[h!]
\centering
\caption{$h$-convergence — mean relative eigenfrequency error [\%] and
         solver time [s] over first """ + str(N_MODES) + r""" modes.}
\label{tab:conv}
\begin{tabular}{crrrcrrrr}
\toprule
$h$ [m] & \multicolumn{3}{c}{CQUAD4} & & \multicolumn{3}{c}{CQUAD8} \\
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.72\textwidth]{validation_convergence.png}
  \caption{$hp$-convergence of the mean eigenfrequency error.
           Dashed lines: theoretical $\mathcal{O}(h^2)$ and
           $\mathcal{O}(h^4)$ slopes.
           Solver time per mesh point is annotated next to each marker.}
  \label{fig:conv}
\end{figure}

%%─────────────────────────────────────────────────────────────────────────────
\section{Comparison with FEniCSx}

pyCAFE results are cross-validated against \textbf{FEniCSx} (DOLFINx v0.9),
an established open-source FEM platform, using the same cavity geometry and
balanced mesh configuration. The objective of this section is twofold:
first, to verify that pyCAFE reproduces the same physical solution; second,
to quantify whether that agreement is obtained at comparable or lower
computational cost.
...
Mesh          & Gmsh structured CQUAD8   & \texttt{create\_rectangle} $""" + str(Nx) + r"""\times """ + str(Ny) + r"""$ \\
DOF (modal)   & """ + str(n_nodes_q8) + r""" & """ + str(n_dof_fenics) + r""" \\
...
Table~\ref{tab:fenics_modal} compares the first """ + str(N_MODES) + r""" eigenfrequencies.
At the balanced configuration, pyCAFE and FEniCSx achieve the same mean
relative error to four significant digits
(""" + fmt(err_cafe.mean()) + r"""\,\% vs """ + fmt(_fenics_modal_err_mean) + r"""\,\%).
However, pyCAFE reaches this accuracy with """ + str(n_nodes_q8) + r""" pressure
unknowns instead of """ + str(n_dof_fenics) + r""" for FEniCSx, i.e.\ about
""" + fmt(dof_reduction_pct, 1) + r"""\,\% fewer DOFs. In addition, the reduced
eigensolve stage in pyCAFE is faster
(""" + fmt(t_cafe_solve, 3) + r""" s vs """ + fmt(t_fenics_solve, 3) + r""" s, a
""" + fmt(solve_speedup, 2) + r"""$\times$ ratio in favour of pyCAFE for this
test case). Figure~\ref{fig:modal} summarises the corresponding normalised
error, DOF-count, and eigensolve-time metrics.

\begin{table}[h!]
\centering
\caption{Eigenfrequencies — pyCAFE CQUAD8 vs FEniCSx Q2 vs analytical
         (balanced mesh $h=""" + fmt(H_BAL, 3) + r"""\,\text{m}$).}
\label{tab:fenics_modal}
\begin{tabular}{crrrrr}
\toprule
Mode $(m,n)$ & $f_\text{an}$ [Hz]
             & $f_\text{FEniCSx}$ [Hz] & $\varepsilon_F$ [\%]
             & $f_\text{pyCAFE}$ [Hz]  & $\varepsilon_P$ [\%] \\
\midrule
""" + "\n".join(_fenics_rows) + r"""
\midrule
\multicolumn{2}{r}{Mean error}
  & \multicolumn{2}{c}{""" + fmt(_fenics_modal_err_mean) + r"""\,\%}
  & \multicolumn{2}{c}{""" + fmt(err_cafe.mean()) + r"""\,\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=\textwidth]{comparison_modal.png}
  \caption{Left: grouped bar chart of the first \num{""" + str(N_MODES) + r"""} eigenfrequencies
           (analytical, FEniCSx Q2, pyCAFE CQUAD8). Right: modal comparison
           summary normalised by the FEniCSx values. Ratios below 1 indicate
           smaller pyCAFE values. At the balanced mesh, the two formulations
           give nearly identical modal errors, while differing in unknown count
           and eigensolve time.}
  \label{fig:modal}
\end{figure}

To complement the eigenfrequency table, Figure~\ref{fig:modeshapes} compares
the corresponding pressure mode shapes for modes 1, 4, 6, and 7 returned by
each solver. Each field is normalised to the range $[-1,1]$ so that the
visual comparison focuses on nodal pattern and phase distribution rather than
absolute scaling. Since eigenvectors are defined only up to a global sign, the
FEniCSx mode is sign-aligned with the corresponding pyCAFE mode before
plotting.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.90\textwidth]{comparison_modeshapes.png}
  \caption{Mode-shape comparison between pyCAFE CQUAD8 and FEniCSx Q2 for
           modes 1, 4, 6, and 7. Each field is independently normalised to
           the interval $[-1,1]$, and the sign of the FEniCSx eigenvector is
           aligned with the pyCAFE one before plotting.}
  \label{fig:modeshapes}
\end{figure}

\subsection{Modal analysis --- pressure-release cavity (4 walls $p=0$)}

As a second modal benchmark, all four walls are assigned a homogeneous
Dirichlet condition ($p = 0$), turning the cavity into a pressure-release
enclosure. For a rectangular cavity $[0,L_x]\times[0,L_y]$, the Helmholtz
equation with homogeneous Dirichlet conditions admits sine-type eigenfunctions
$\phi_{mn}(x,y)=\sin(m\pi x/L_x)\sin(n\pi y/L_y)$, which satisfy $p=0$ on
every wall provided $m,n\ge 1$. The wavenumber condition
$k_{mn}^2=(m\pi/L_x)^2+(n\pi/L_y)^2$ yields
\begin{equation}
  f_{mn} = \frac{c_0}{2}\sqrt{\left(\frac{m}{L_x}\right)^2
           + \left(\frac{n}{L_y}\right)^2}, \quad m,n \ge 1.
\end{equation}
Although the \emph{formula} is identical to the rigid-cavity (Neumann) case,
the two benchmarks differ in the admissible index set: hard walls allow
$m=0$ or $n=0$ (planar modes whose gradient vanishes on the corresponding
pair of walls), whereas pressure-release walls require both $m\ge 1$ and
$n\ge 1$ (the field itself must vanish on every wall). The lowest
pressure-release eigenfrequency is therefore the $(1,1)$ mode, roughly
$383$\,Hz for the present geometry, whereas the rigid cavity begins at the
$(1,0)$ mode near $172$\,Hz.

In pyCAFE this BC is handled by the built-in Dirichlet reduction (boundary
nodes eliminated from the system before the eigensolve). In FEniCSx the same
reduction is applied via index extraction before calling the sparse
eigenvalue solver.

Table~\ref{tab:fenics_modal_zp} lists the first """ + str(N_MODES) + r""" eigenfrequencies.
Both solvers achieve very low errors (pyCAFE mean $\varepsilon=\,$""" + fmt(err_cafe_zp.mean()) + r"""\,\%,
FEniCSx mean $\varepsilon=\,$""" + fmt(err_fenics_zp.mean()) + r"""\,\%),
with pyCAFE requiring """ + str(sys_m_zp["K_red"].shape[0]) + r""" reduced DOFs against
""" + str(len(_free_dofs_zp)) + r""" free DOFs for FEniCSx.
Figure~\ref{fig:modal_zp} mirrors the summary chart from the rigid-wall case.

\begin{table}[h!]
\centering
\caption{Eigenfrequencies --- pressure-release cavity ($p=0$ on all walls),
         pyCAFE CQUAD8 vs FEniCSx Q2 vs analytical
         ($h=""" + fmt(H_BAL, 3) + r"""\,\text{m}$).}
\label{tab:fenics_modal_zp}
\begin{tabular}{crrrrr}
\toprule
Mode $(m,n)$ & $f_\text{an}$ [Hz]
             & $f_\text{FEniCSx}$ [Hz] & $\varepsilon_F$ [\%]
             & $f_\text{pyCAFE}$ [Hz]  & $\varepsilon_P$ [\%] \\
\midrule
""" + "\n".join(_fenics_rows_zp) + r"""
\midrule
\multicolumn{2}{r}{Mean error}
  & \multicolumn{2}{c}{""" + fmt(err_fenics_zp.mean()) + r"""\,\%}
  & \multicolumn{2}{c}{""" + fmt(err_cafe_zp.mean()) + r"""\,\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=\textwidth]{comparison_modal_zero_pressure.png}
  \caption{Left: grouped bar chart of the first \num{""" + str(N_MODES) + r"""} eigenfrequencies
           for the pressure-release cavity (analytical, FEniCSx Q2, pyCAFE
           CQUAD8). Right: metrics normalised by FEniCSx values.}
  \label{fig:modal_zp}
\end{figure}

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.90\textwidth]{comparison_modeshapes_zero_pressure.png}
  \caption{Mode-shape comparison between pyCAFE CQUAD8 and FEniCSx Q2 for
           modes 1, 2, 4, and 6 of the \textbf{pressure-release cavity}
           ($p=0$ on all 4 walls). Normalisation and sign alignment as in
           Figure~\ref{fig:modeshapes}.}
  \label{fig:modeshapes_zp}
\end{figure}

\subsection{Direct Helmholtz frequency sweep}

The direct harmonic problem uses a vibrating piston on the left wall
($v_n = """ + fmt(v_n, 1) + r"""\,\text{m/s}$, imposed normal velocity)
with all other walls rigid.
A frequency sweep from 300\,Hz to 700\,Hz (1\,Hz step) is performed and the
complex acoustic pressure is recorded at three probe locations:
\begin{itemize}
  \item right-wall centre $(L_x,\,L_y/2)$,
  \item top-right corner $(L_x,\,L_y)$,
  \item cavity centre $(L_x/2,\,L_y/2)$.
\end{itemize}
Figure~\ref{fig:direct} overlays the frequency response functions (solid:
pyCAFE~CQUAD8, dashed: FEniCSx~Q2) for all three probes. The purpose of this
test is primarily solution agreement: the mean relative mismatch in pressure
magnitude over the three probe responses is """ + fmt(mean_probe_rel_diff, 3) + r"""\,\%.
The sweep timings are reported for completeness
(pyCAFE """ + f"{t_cafe_sweep:.2f}" + r"""\,s, FEniCSx """ + f"{t_fenics_sweep:.2f}" + r"""\,s),
but the figure is interpreted mainly as a cross-validation of the predicted
frequency response.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.92\textwidth]{comparison_direct.png}
  \caption{Pressure magnitude vs frequency at three probe locations — pyCAFE
           CQUAD8 (solid) vs FEniCSx Q2 (dashed). Piston excitation on the
           left wall, $v_n=""" + fmt(v_n, 1) + r"""\,\text{m/s}$. The close
           overlay confirms direct-frequency-response agreement between the
           two implementations; timing information is reported in the inset.}
  \label{fig:direct}
\end{figure}

\subsection{Probe responses for different boundary conditions}

To compare the direct-frequency response under different boundary-condition
types, frequency sweeps are computed for two configurations:
prescribed normal velocity and pressure excitation with impedance termination.
The response is extracted at the cavity-centre probe
$(L_x/2, L_y/2)$, and the absolute value, real part, and phase of the complex
pressure are exported as separate images.

""" + "\n".join(field_case_tex_blocks) + r"""

%%─────────────────────────────────────────────────────────────────────────────
\begin{thebibliography}{9}
\bibitem{Kinsler2000}
L.~E. Kinsler, A.~R. Frey, A.~B. Coppens, J.~V. Sanders,
\textit{Fundamentals of Acoustics}, 4th ed.\ Wiley, 2000.
\bibitem{Ihlenburg1998}
F.~Ihlenburg,
\textit{Finite Element Analysis of Acoustic Scattering}.
Springer, 1998.
...
\bibitem{Dolfinx2023}
I.~A. Baratta et al.,
\textit{DOLFINx: The next generation FEniCS problem solving environment}.
Zenodo, 2023. \texttt{doi:10.5281/zenodo.10447666}.
\end{thebibliography}
...
tex_path = OUT_DIR / "validation_pycafe.tex"
