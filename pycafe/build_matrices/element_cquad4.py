...
#   3D -> Local system
def calcola_cquad4(x1, x2, x3, x4):
    ...
    nodi = np.vstack([x1, x2, x3, x4])  # (4,3)
...
def element_matrices_cquad4(x_e, c, quad_rule=gauss_rule_quad_2x2()):
    """
    Compute the element stiffness and mass matrices for a CQUAD4 element.

    This function computes the local acoustic stiffness matrix (K_e)
    and mass matrix (M_e) for a 4-node quadrilateral (CQUAD4) element
    using numerical integration.

    Parameters
    ----------
    x_e : ndarray of shape (4, 2) or (4, 3)
    ...
    quad_rule : tuple, optional
        ...
        Default is the 2×2 Gauss quadrature rule.
    ...
    The formulation assumes linear acoustic pressure interpolation
    over the CQUAD4 element.

    See Also
    --------
    cquad4_shape : Shape functions and derivatives for CQUAD4 elements.
    jacobian_2d : Compute the Jacobian of the isoparametric mapping.
    gauss_rule_quad_2x2 : Default Gauss quadrature rule.
    """
    ...
    K_e = np.zeros((4, 4), dtype=float)
    M_e = np.zeros((4, 4), dtype=float)

    for xi, eta, w in zip(xi_pts, eta_pts, w_pts):
        N, dN_dxi, dN_deta = cquad4_shape(xi, eta)
